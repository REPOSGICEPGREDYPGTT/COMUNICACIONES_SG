...
err_t ccl_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
  ...
  else if(es->state == ES_ACCEPTED)
  {
    ...
    if(RMS_RE>=1 && (RMS_GD1<1 && RMS_GD2<1)){
        uint8_t x[] ={"10000000000000,1,0.14,0.02,0,1.20,0.010,0.090,002,1,0.14,0.02,0,1.00,0.011,0.060,003,1,0.14,0.02,0,0.53,0.011,0.015,005,1,0.14,0.02,0,2.00,0.200,10.00,100,1,0.14,0.02,0,2.00,0.200,10.00,100,"};
        y=x;
        }

    //Red con GD:
    if(RMS_RE<1 && (RMS_GD1>=1 || RMS_GD2>=1)){
        uint8_t x[] ={"20000000000000,2,0.14,0.02,0,1.25,0.030,0.100,005,2,0.14,0.02,0,1.25,0.020,0.060,007,2,0.14,0.02,0,0.65,0.016,0.040,008,2,0.14,0.02,0,1.10,0.010,0.015,006,2,0.14,0.02,0,1.70,0.005,0.010,003,"};
        y=x;
        }
...
void ccl_send(struct tcp_pcb *tpcb, struct ccl_state *es)
{
  ...
  {
  wr_err = tcp_write(tpcb, es->ensayo, 190, 1);
  tcp_output(tpcb);
