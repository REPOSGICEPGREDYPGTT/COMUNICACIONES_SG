...
err_t ccl_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
  ...
  else if(es->state == ES_ACCEPTED)
  {
    ...
    //DATOS DE CORRIENTE RMS 3F - CHEC

    uint32_t RMS_3F;
    
    RMS_3F = *(pData); //Acceso a cada byte
    

    //Lógica de la función de protección 50/51
    uint8_t *y;

    int Iin[1];
    Iin[0] = RMS_3F;

    //ResetProt(OCP.Trig, &OCP);
    OCP50_Loop(&OCP, Iin);
    if (OCP.Trig == 1)
    {
        uint8_t x[] = { "1" };
...
void ccl_send(struct tcp_pcb *tpcb, struct ccl_state *es)
{
  ...
  {
  wr_err = tcp_write(tpcb, es->ensayo, 1, 1);
  tcp_output(tpcb);
